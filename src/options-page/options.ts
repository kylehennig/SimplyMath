...
window.addEventListener("load", () => {
  ...
  const fontSizeInput = getInput("fontSizeInput");
  const maxDepthInput = getInput("maxDepthInput");
  const spaceBehavesLikeTab = getInput("spaceBehavesLikeTab");
  const restrictMismatchedBrackets = getInput("restrictMismatchedBrackets");
  const sumStartsWithNEquals = getInput("sumStartsWithNEquals");
  const supSubsRequireOperand = getInput("supSubsRequireOperand");
  const autoSubscriptNumerals = getInput("autoSubscriptNumerals");
